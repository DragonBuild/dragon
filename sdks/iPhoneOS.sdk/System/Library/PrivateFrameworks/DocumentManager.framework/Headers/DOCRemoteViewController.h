//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Mar 22 2020 01:47:48).
//
//  Copyright (C) 1997-2019 Steve Nygard.
//

#import <UIKitCore/_UIRemoteViewController.h>

#import <DocumentManager/DOCRemoteContextInterface-Protocol.h>

@class DOCAppearance, DOCConfiguration, DOCRemoteContext;
@protocol DOCRemoteViewControllerDelegate;

@interface DOCRemoteViewController : _UIRemoteViewController <DOCRemoteContextInterface>
{
    BOOL _isBrowserViewController;
    id <DOCRemoteViewControllerDelegate> _delegate;
    DOCRemoteContext *_hostContext;
    DOCConfiguration *_configuration;
    DOCAppearance *_lastAppearance;
}

+ (id)serviceExtension;
+ (struct _UIResilientRemoteViewContainerViewController )instantiateRemoteTargetSelectionBrowserViewControllerWithHostProxy:(id)arg1 configuration:(id)arg2 completionBlock:(id /* CDUnknownBlockType */)arg3;
+ (struct _UIResilientRemoteViewContainerViewController )instantiateRemoteDocumentBrowserViewControllerWithHostProxy:(id)arg1 configuration:(id)arg2 completionBlock:(id /* CDUnknownBlockType */)arg3;
+ (struct _UIResilientRemoteViewContainerViewController )instantiateRemoteViewControllerWithConfiguration:(id)arg1 transparent:(BOOL)arg2 errorHandler:(id /* CDUnknownBlockType */)arg3 hostProxy:(id)arg4 completionHandler:(id /* CDUnknownBlockType */)arg5;
@property(retain) DOCAppearance *lastAppearance; // @synthesize lastAppearance=_lastAppearance;
@property BOOL isBrowserViewController; // @synthesize isBrowserViewController=_isBrowserViewController;
@property(retain) DOCConfiguration *configuration; // @synthesize configuration=_configuration;
@property(retain) DOCRemoteContext *hostContext; // @synthesize hostContext=_hostContext;
@property __weak id <DOCRemoteViewControllerDelegate> delegate; // @synthesize delegate=_delegate;
// - (void).cxx_destruct;
- (void)viewServiceDidTerminateWithError:(id)arg1;
- (void)updateAppearance:(id)arg1 shouldFlushCA:(BOOL)arg2 completionBlock:(id /* CDUnknownBlockType */)arg3;
- (void)updateAppearance:(id)arg1 completionBlock:(id /* CDUnknownBlockType */)arg2;
- (void)updateEditingTo:(BOOL)arg1 animated:(BOOL)arg2;
- (void)viewDidLoad;

@end

