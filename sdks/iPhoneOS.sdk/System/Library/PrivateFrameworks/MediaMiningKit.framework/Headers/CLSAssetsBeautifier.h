//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Mar 22 2020 01:47:48).
//
//  Copyright (C) 1997-2019 Steve Nygard.
//

#import <objc/NSObject.h>

@class CLSSimilarStacker, NSSet;

@interface CLSAssetsBeautifier : NSObject
{
    BOOL _enableStatisticalSampling;
    BOOL _enableNetworkAccess;
    BOOL _enableIntermediateNaturalClustering;
    BOOL _enableFinalNaturalClustering;
    BOOL _enableFinalTimeClustering;
    BOOL _usesVision;
    CLSSimilarStacker *_similarStacker;
    BOOL _usesKMeans;
    NSSet *_identifiersOfRequiredItems;
}

+ (id)beautifier;
@property(copy, nonatomic) NSSet *identifiersOfRequiredItems; // @synthesize identifiersOfRequiredItems=_identifiersOfRequiredItems;
@property(nonatomic) BOOL usesKMeans; // @synthesize usesKMeans=_usesKMeans;
@property(nonatomic) BOOL usesVision; // @synthesize usesVision=_usesVision;
@property(nonatomic) BOOL enableFinalTimeClustering; // @synthesize enableFinalTimeClustering=_enableFinalTimeClustering;
@property(nonatomic) BOOL enableFinalNaturalClustering; // @synthesize enableFinalNaturalClustering=_enableFinalNaturalClustering;
@property(nonatomic) BOOL enableIntermediateNaturalClustering; // @synthesize enableIntermediateNaturalClustering=_enableIntermediateNaturalClustering;
@property(nonatomic) BOOL enableNetworkAccess; // @synthesize enableNetworkAccess=_enableNetworkAccess;
@property(nonatomic) BOOL enableStatisticalSampling; // @synthesize enableStatisticalSampling=_enableStatisticalSampling;
// - (void).cxx_destruct;
- (id)requiredItemsInItems:(id)arg1;
- (BOOL)itemIsRequired:(id)arg1;
- (id)rankSimilarItems:(id)arg1;
- (id)_naturalClusteringWithItems:(id)arg1 withSimilarity:(long long)arg2 timestampSupport:(BOOL)arg3 debugInfo:(id)arg4;
- (id)_naturalClusteringBestItemInItems:(id)arg1;
- (id)bestItemInItems:(id)arg1;
- (id)sortedItemsWithItems:(id)arg1;
- (id)_clustersBySplittingZeroDiameterClustersInClusters:(id)arg1 targetingNumberOfClusters:(NSUInteger)arg2;
- (id)deduplicateItems:(id)arg1 withDuration:(double)arg2 andSimilarity:(long long)arg3 debugInfo:(id)arg4;
- (id)deduplicateItems:(id)arg1 debugInfo:(id)arg2;
- (id)performWithItems:(id)arg1 maximumNumberOfItemsToChoose:(NSUInteger)arg2 debugInfo:(id)arg3 progressBlock:(id /* CDUnknownBlockType */)arg4;
- (id)performWithItems:(id)arg1 maximumNumberOfItemsToChoose:(NSUInteger)arg2 progressBlock:(id /* CDUnknownBlockType */)arg3;
- (id)sampledItemsInSortedItems:(id)arg1 maximumNumberOfItemsToChoose:(NSUInteger)arg2 debugInfo:(id)arg3 progressBlock:(id /* CDUnknownBlockType */)arg4;
- (id)init;
- (id /* CDUnknownBlockType */)timeClusteringDistanceBlock;
- (id /* CDUnknownBlockType */)hierarchicalClusteringDistanceBlock;

@end

